...
function redConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    ...
    for (let j = 0; j < pixels[i].length; j++) {
      ...
      nuevaFila.push([R, 0, 0]); 
      
    }
    nuevoPixel.push(nuevaFila);
  }

  handler.savePixels(nuevoPixel, outputPath);
}
...
function blackAndWhiteConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    ...
    for (let j = 0; j < pixels[i].length; j++) {
      ...
      let pixelBlackWhite;
      if (avg < 128) {
        pixelBlackWhite = [0, 0, 0]; 
      } else {
        pixelBlackWhite = [255, 255, 255]; 
      }
      nuevaFila.push(pixelBlackWhite);
    }
    nuevoPixel.push(nuevaFila);
  }
...
function scaleDown() {
  ...
  for (let i = 0; i < pixels.length; i += 2) { 
    ...
    pixel.push(nuevaFila);
  }

  handler.savePixels(pixel, outputPath, handler.getShape()[0] / 2, handler.getShape()[1] / 2);
}
...
function dimBrightness(dimFactor) {
  ...
  for (let i = 0; i < pixels.length; i++) {
    ...
    for (let j = 0; j < pixels[i].length; j++) {
      let [R, G, B] = pixels[i][j];
      nuevaFila.push([
        Math.max(0, Math.floor(R / dimFactor)),
        Math.max(0, Math.floor(G / dimFactor)),
        Math.max(0, Math.floor(B / dimFactor))
      ]);
    }
    pixel.push(nuevaFila);
  }

  handler.savePixels(pixel, outputPath);
}
...
function invertColors() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    ...
    for (let j = 0; j < pixels[i].length; j++) {
      let [R, G, B] = pixels[i][j];
      nuevaFila.push([255 - R, 255 - G, 255 - B]); 
    }
    pixel.push(nuevaFila);
  }
