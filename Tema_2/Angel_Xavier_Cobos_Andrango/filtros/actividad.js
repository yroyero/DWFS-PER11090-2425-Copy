...
function greyConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      media = (pixels[i][j][1] + pixels[i][j][2] + pixels[i][j][0]) / 3;
      pixels[i][j][0] = media;
      pixels[i][j][1] = media;
      pixels[i][j][2] = media;
    }
...
function scaleDown() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    ...
    for (let j = 0; j < pixels[i].length; j++) {
      if(j%2 == 0 && i%2 == 0){
        auxPixel.push(pixels[i][j]);
      }
    }
    if(auxPixel.length > 0){
      newPixels.push(auxPixel);
    }
  ...
  for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      ...
      for (let k = 0; k < catPixels[i][j].length; k++) {
        newPixel.push(catPixels[i][j][k] * alphaFirst + dogPixels[i][j][k] * alphaSecond);
      }
