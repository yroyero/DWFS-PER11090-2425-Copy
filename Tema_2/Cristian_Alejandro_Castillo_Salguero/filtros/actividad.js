...
function redConverter() {
  ...
  for(let i = 0; i < pixels.length; i++){
      for(let j = 0; j < pixels[i].length; j++){
        let tempPixel = pixels[i][j];
        pixels[i][j] = [tempPixel[0], 0, 0];
      }
  }
...
function greyConverter() {
  ...
  for(let i = 0; i < pixels.length; i++){
    for(let j = 0; j < pixels[i].length; j++){
      let tempPixel = pixels[i][j];
      let media = (tempPixel[0] + tempPixel[1] + tempPixel[2]) / 3;
...
function scaleDown() {
  ...
  handler.savePixels(pixeles2, outputPath, handler.getShape()[0] / 2, handler.getShape()[1] / 2);
}
...
function dimBrightness(dimFactor) {
  ...
  for(i = 0; i < pixels.length; i++){
    for(j = 0; j < pixels[i].length; j++){
      let tempPixel = pixels[i][j];
      pixels[i][j] = [tempPixel[0]/dimFactor, tempPixel[1]/dimFactor, tempPixel[2]/dimFactor];
    }
  }
...
function invertColors() {
  ...
  for(i = 0; i < pixels.length; i++){
    for(j = 0; j < pixels[i].length; j++){
      let tempPixel = pixels[i][j];
      pixels[i][j] = [255 - tempPixel[0], 255 - tempPixel[1], 255 - tempPixel[2]];
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for(i = 0; i < dogPixels.length; i++){

    let fila = []
    for(j = 0; j < dogPixels[i].length; j++){

      let catPixel = catPixels[i][j];
      let dogPixel = dogPixels[i][j];

      let pixel = [(catPixel[0] * alphaFirst) + (dogPixel[0] * alphaSecond), (catPixel[1] * alphaFirst) + (dogPixel[1] * alphaSecond), (catPixel[2] * alphaFirst) + (dogPixel[2] * alphaSecond)];
