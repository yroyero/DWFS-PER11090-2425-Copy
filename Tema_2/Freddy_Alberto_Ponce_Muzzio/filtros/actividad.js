...
function redConverter() {
  ...
  for (let i = 0; i < filas; i++) {
    for (let j = 0; j < columnas; j++) {
      let nuevoPixel = [pixels[i][j][0], 0, 0]
      pixels[i][j] = nuevoPixel
    }
  }
...
function greenConverter() {
  ...
  for (let i = 0; i < filas; i++) {
    for (let j = 0; j < columnas; j++) {
      let nuevoPixel = [0, pixels[i][j][1], 0]
      pixels[i][j] = nuevoPixel
    }
  }
...
function blueConverter() {
  ...
  for (let i = 0; i < filas; i++) {
    for (let j = 0; j < columnas; j++) {
      let nuevoPixel = [0, 0, pixels[i][j][2]]
      pixels[i][j] = nuevoPixel
    }
  }
...
function greyConverter() {
  ...
  for (let i = 0; i < filas; i++) {
    for (let j = 0; j < columnas; j++) {
      let initialStatus = 0
      let media = pixels[i][j].reduce((accumulator, currentValue) => accumulator + currentValue, initialStatus)/3
      let nuevoPixel = Array(3).fill(media)
      pixels[i][j] = nuevoPixel
    }
  }
...
function blackAndWhiteConverter() {
  ...
  for (let i = 0; i < filas; i++) {
    for (let j = 0; j < columnas; j++) {
      let initialStatus = 0
      let media = pixels[i][j].reduce((accumulator, currentValue) => accumulator + currentValue, initialStatus)/3
      let nuevoPixel = Array(3).fill(255)
      if (media<128){
        nuevoPixel = Array(3).fill(0)
      }
      pixels[i][j] = nuevoPixel
    }
  }
...
function scaleDown() {
  ...
  handler.savePixels(nuevaMatriz, outputPath, handler.getShape()[0] / 2, handler.getShape()[1] / 2);
}
...
function dimBrightness(dimFactor) {
  ...
  for (let i = 0; i < filas; i++) {
    for (let j = 0; j < columnas; j++) {
      let nuevoPixel = pixels[i][j].map((pixel) => pixel/dimFactor)
      pixels[i][j] = nuevoPixel
    }
  }
...
function invertColors() {
  ...
  for (let i = 0; i < filas; i++) {
    for (let j = 0; j < columnas; j++) {
      nuevoPixel = pixels[i][j].map((pixel) => 255 - pixel)
      pixels[i][j] = nuevoPixel
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i++) {
    let nuevaFila = []
    for (let j = 0; j < catPixels[0].length; j++) {
      let dogPixelMultiplied = dogPixels[i][j].map((pixel) => pixel*alphaFirst)
      let catPixelMultiplied = catPixels[i][j].map((pixel) => pixel*alphaSecond)
      let nuevoPixel = catPixelMultiplied.map((pixel, index) => pixel + dogPixelMultiplied[index])
