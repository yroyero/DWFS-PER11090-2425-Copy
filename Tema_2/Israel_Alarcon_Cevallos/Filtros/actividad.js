...
function greyConverter() {
  ...
  for (let i=0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      for (let k = 0; k < 3; k++) {
        color = color + pixels[i][j][k];
      }
      pixels[i][j][0] = color/3;
      pixels[i][j][1] = color/3;
      pixels[i][j][2] = color/3;
      color =  0;
    }
  }
  handler.savePixels(pixels, outputPath);
...
function dimBrightness(dimFactor) {
  ...
  for (let i=0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      ...
      pixels[i][j][0] = R/dimFactor;
      pixels[i][j][1] = G/dimFactor;
      pixels[i][j][2] = B/dimFactor;
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i=0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      let R = catPixels[i][j][0]*alphaFirst;
      let G = catPixels[i][j][1]*alphaFirst;
      let B = catPixels[i][j][2]*alphaFirst;

      let R1= dogPixels[i][j][0]*alphaSecond;
      let G1= dogPixels[i][j][1]*alphaSecond;
      let B1= dogPixels[i][j][2]*alphaSecond;

      nuevafila.push([R+R1,G+G1,B+B1])
    }
