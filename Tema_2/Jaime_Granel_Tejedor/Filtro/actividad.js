...
function redConverter() {
  ...
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let columna = 0; columna < pixels[fila].length; columna++) {
      pixels[fila][columna] = [pixels[fila][columna][0],0,0];
    }
  }
...
function greenConverter() {
  ...
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let columna = 0; columna < pixels[fila].length; columna++) {
      pixels[fila][columna] = [0,pixels[fila][columna][1],0];
    }
  }
...
function blueConverter() {
  ...
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let columna = 0; columna < pixels[fila].length; columna++) {
      pixels[fila][columna] = [0,0,pixels[fila][columna][2]];
    }
  }
...
function greyConverter() {
  ...
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let columna = 0; columna < pixels[fila].length; columna++) {
      ...
      pixels[fila][columna]= [media,media,media];
    }
  }
...
function dimBrightness(dimFactor) {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      ...
      pixels[i][j] = pixel.map(channel => Math.max(Math.min(Math.floor(channel / dimFactor), 255), 0));
    }
  }
...
function invertColors() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      let pixel = pixels[i][j];
      pixels[i][j] = pixel.map(channel => 255 - channel);
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  let pixels = [];
  if (
      catHandler.getShape()[0] !== dogHandler.getShape()[0] ||
      catHandler.getShape()[1] !== dogHandler.getShape()[1]
  ) {
    throw new Error('Las imágenes deben tener las mismas dimensiones para fusionarse.');
  }
  ...
  for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      ...
      let mergedPixel = catPixel.map((catValue, channelIndex) =>
          Math.round(
              alphaFirst * catValue + alphaSecond * dogPixel[channelIndex]
          )
      );
