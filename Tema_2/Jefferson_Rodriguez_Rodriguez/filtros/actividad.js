...
function scaleDown() {
  ...
  for (let i = 0; i < pixels.length; i += 2) {
    ...
    scaledImage.push(newRow);
  }

  handler.savePixels(scaledImage, outputPath, handler.getShape()[0] / 2, handler.getShape()[1] / 2);
}
...
function dimBrightness(dimFactor) {
  ...
  for (let row of pixels) {
    for (let pixel of row) {
      pixel[0] = Math.max(0, Math.min(255, Math.round(pixel[0] / dimFactor)));
      pixel[1] = Math.max(0, Math.min(255, Math.round(pixel[1] / dimFactor)));
      pixel[2] = Math.max(0, Math.min(255, Math.round(pixel[2] / dimFactor)));
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      ...
      let red = Math.round(catPixel[0] * alphaFirst + dogPixel[0] * alphaSecond);
      let green = Math.round(catPixel[1] * alphaFirst + dogPixel[1] * alphaSecond);
      let blue = Math.round(catPixel[2] * alphaFirst + dogPixel[2] * alphaSecond);

      let mergedPixel = [red, green, blue];
