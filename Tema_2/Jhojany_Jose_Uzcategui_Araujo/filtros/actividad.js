...
function greyConverter() {
  ...
  const newPixels = pixels.map((row) => {
    return row.map((col) => {
      const suma = col.reduce((sum, numero) => sum + numero, 0);
      const prom = suma / col.length;
...
function blackAndWhiteConverter() {
  ...
  const newPixels = pixels.map((row) => {
    return row.map((col) => {
      const suma = col.reduce((sum, numero) => sum + numero, 0);
      const prom = suma / col.length;
      return prom < 128? [0, 0, 0] : [255, 255, 255];
    })
  })
  pixels = newPixels;
  handler.savePixels(pixels, outputPath);
...
function scaleDown() {
  ...
  const newPixels = pixels.map((row, rowIndex) => {
    if (rowIndex % 2 === 0) return null;
    return row
      .map((col, colIndex) => (colIndex % 2 === 0 ? null : col))
      .filter(col => col !== null);
  }).filter(row => row !== null);
  pixels = newPixels;
...
function dimBrightness(dimFactor) {
  ...
  const newPixels = pixels.map((row) => {
    return row.map((col) => {
      return [Math.trunc(col[0] / dimFactor), Math.trunc(col[1] / dimFactor), Math.trunc(col[2] / dimFactor)];
    })
  })
  pixels = newPixels;
  handler.savePixels(pixels, outputPath);
...
function merge(alphaFirst, alphaSecond) {
  ...
  //Aqui tu codigo
  pixels = catPixels.map((row, rowIdex) => {
    return row.map((col, colIndex) => {
      return [
        Math.trunc(col[0] * alphaSecond) + Math.trunc(dogPixels[rowIdex][colIndex][0] * alphaFirst),
        Math.trunc(col[1] * alphaSecond) + Math.trunc(dogPixels[rowIdex][colIndex][1] * alphaFirst),
        Math.trunc(col[2] * alphaSecond) + Math.trunc(dogPixels[rowIdex][colIndex][2] * alphaFirst)
      ]
    })
  })
