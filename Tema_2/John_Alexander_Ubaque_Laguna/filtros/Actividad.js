...
function greyConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
        let average = (pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2]) / 3;
...
function blackAndWhiteConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      let average = (pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2]) / 3;
      let pixelColorNewValue = average<128 ? 0:255;

      pixels[i][j][0] = pixelColorNewValue;
      pixels[i][j][1] = pixelColorNewValue;
      pixels[i][j][2] = pixelColorNewValue;
    }
  }
  handler.savePixels(pixels, outputPath);
}
...
function scaleDown() {
  ...
  handler.savePixels(newPixels, outputPath, handler.getShape()[0] / 2, handler.getShape()[1] / 2);  handler.savePixels(newPixels, outputPath, newPixels.length, newPixels[0].length);
}
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      let newPixel = [
        Math.round(catPixels[i][j][0] * alphaFirst + dogPixels[i][j][0] * alphaSecond),
        Math.round(catPixels[i][j][1] * alphaFirst + dogPixels[i][j][1] * alphaSecond),
        Math.round(catPixels[i][j][2] * alphaFirst + dogPixels[i][j][2] * alphaSecond)
      ];
