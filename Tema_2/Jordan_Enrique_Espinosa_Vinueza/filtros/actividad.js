...
function greyConverter() {
    ...
    for (let i = 0; i < pixels.length; i++){
        for (let j= 0; j<pixels[i].length; j++){
            let promedio = Math.floor((pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2]) /3);
            pixels[i][j][0] = promedio; // R
            pixels[i][j][1] = promedio; // G
            pixels[i][j][2] = promedio; // B
        }
    }
...
function blackAndWhiteConverter() {
    ...
    for (let i = 0; i < pixels.length; i++){
        for (let j= 0; j<pixels[i].length; j++){
            ...
            if (promedio < 128){
                pixels[i][j] = [0,0,0]; //negro
            }else{
                pixels [i][j] = [255, 255, 255]; //Blanco
            }
...
function dimBrightness(dimFactor) {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            ...
            pixel[0] = Math.max(0, Math.floor(pixel[0] / dimFactor)); // R
            pixel[1] = Math.max(0, Math.floor(pixel[1] / dimFactor)); // G
            pixel[2] = Math.max(0, Math.floor(pixel[2] / dimFactor)); // B
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let i = 0; i < dogPixels.length; i++) {
        ...
        for (let j = 0; j < dogPixels[i].length; j++) {
            let pixelR = dogPixels[i][j][0] * alphaFirst + catPixels[i][j][0] * alphaSecond;
