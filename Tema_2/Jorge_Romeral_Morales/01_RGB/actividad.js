...
function blackAndWhiteConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      let media = (pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2]) / 3;
      media < 128 ? pixels[i][j] = [0, 0, 0] : pixels[i][j] = [255, 255, 255];
    }
  }
...
function dimBrightness(dimFactor) {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      pixels[i][j] = pixels[i][j].map(value => Math.round(value / dimFactor)); 
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      ...
      let pixel = [
        Math.round(catPixels[i][j][0]*alphaSecond + dogPixels[i][j][0]*alphaFirst),
        Math.round(catPixels[i][j][1]*alphaSecond + dogPixels[i][j][1]*alphaFirst),
        Math.round(catPixels[i][j][2]*alphaSecond + dogPixels[i][j][2]*alphaFirst),
      ];

      combinedPixels.push(pixel);
    }
  
    pixels.push(combinedPixels);
  }
