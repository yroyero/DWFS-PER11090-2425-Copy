 ...
 *
 * Una forma de conseguirlo es simplemente poner los canales G y B a 0 para cada pixel. R G B
 */
    ...
    for (let i = 0; i < catPixels.length; i++) {
        ...
        for (let j = 0; j < catPixels[i].length; j++) {
            ...
            for (let k = 0; k < catPixels[i][j].length; k++) {
                pixel.push(catPixels[i][j][k] * alphaSecond + dogPixels[i][j][k] * alphaFirst);
             }
             row.push(pixel);
        }
        pixels.push(row);
