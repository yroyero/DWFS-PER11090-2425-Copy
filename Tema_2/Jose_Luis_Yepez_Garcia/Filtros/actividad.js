...
function greyConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      let media = Math.floor((pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2]) / 3);
      pixels[i][j][0] = media;
      pixels[i][j][1] = media;
      pixels[i][j][2] = media;
    }
...
function scaleDown() {
  ...
  //Aqui tu codigo
  // pixels = pixels.filter((_, index) => index % 2 === 0).map(row => row.filter((_, index) => index % 2 === 0));
  let newPixels = [];
  for (let i = 0; i < pixels.length; i++) {
    let newRow = [];
    for (let j = 0; j < pixels[i].length; j++) {
      if (i % 2 === 0 && j % 2 === 0) newRow.push(pixels[i][j]);
    }
...
function dimBrightness(dimFactor) {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      pixels[i][j][0] = Math.floor(pixels[i][j][0] / dimFactor);
      pixels[i][j][1] = Math.floor(pixels[i][j][1] / dimFactor);
      pixels[i][j][2] = Math.floor(pixels[i][j][2] / dimFactor);
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      let newPixel = [];
      for (let k = 0; k < 3; k++) {
        newPixel.push(Math.floor(catPixels[i][j][k] * alphaFirst + dogPixels[i][j][k] * alphaSecond));
      }
