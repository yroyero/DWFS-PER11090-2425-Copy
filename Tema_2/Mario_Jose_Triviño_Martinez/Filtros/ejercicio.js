...
function invertColors() {
    ...
    for(let i=0;i<pixels.length;i++){
        for(let j=0;j<pixels[i].length;j++){
            ...
            let r = 255 - pixels[i][j][0];
            let g = 255 - pixels[i][j][1];
            let b = 255 - pixels[i][j][2];
...
function merge(alphaFirst, alphaSecond) {
    ...
    for(let i=0;i<catPixels.length;i++){
        ...
        for(let j=0;j<catPixels[i].length;j++){
            ...
            let rCat = catPixels[i][j][0]*alphaSecond;
            let gCat = catPixels[i][j][1]*alphaSecond;
            let bCat = catPixels[i][j][2]*alphaSecond;

            let rDog = dogPixels[i][j][0]*alphaFirst;
            let gDog = dogPixels[i][j][1]*alphaFirst;
            let bDog = dogPixels[i][j][2]*alphaFirst;
            line.push([rCat+rDog, gCat+gDog, bCat+bDog]);
        }
        pixels.push(line);
    }
