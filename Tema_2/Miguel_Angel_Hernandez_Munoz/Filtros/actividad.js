...
function greyConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            ...
            pixels[i][j][0] = media; // RED
            pixels[i][j][1] = media; // GREEN
            pixels[i][j][2] = media; // BLUE
        }
    }
...
function scaleDown() {
    ...
    for (let i = pixels.length - 1; i >= 0; i--) {
        if (i % 2 !== 0) {
            pixels.splice(i, 1); // Eliminar fila impar
        }
    }
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = pixels[i].length - 1; j >= 0; j--) {
            if (j % 2 !== 0) {
                pixels[i].splice(j, 1); // Eliminar columna impar
            }
        }
    }
...
function dimBrightness(dimFactor) {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            pixels[i][j][0] = pixels[i][j][0] /2; // RED
            pixels[i][j][1] = pixels[i][j][1]/2; // GREEN
            pixels[i][j][2] = pixels[i][j][2]/2; // BLUE
        }
    }
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let i = 0; i < catPixels.length; i++) {
        ...
        for (let j = 0; j < catPixels[i].length; j++) {
            ...
            let mergedPixel = [
                Math.min(255, Math.round(catPixel[0] * alphaFirst + dogPixel[0] * alphaSecond)), // RED
                Math.min(255, Math.round(catPixel[1] * alphaFirst + dogPixel[1] * alphaSecond)), // GREEN
                Math.min(255, Math.round(catPixel[2] * alphaFirst + dogPixel[2] * alphaSecond)), // BLUE
            ];
