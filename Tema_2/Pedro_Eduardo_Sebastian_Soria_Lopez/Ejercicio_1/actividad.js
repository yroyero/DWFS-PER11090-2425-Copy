...
function dimBrightness(dimFactor) {
    ...
    for (let columna = 0; columna < pixels[fila].length; columna++) {
        let pixel = pixels[fila][columna];    
        pixel[0] = Math.max(0, Math.round(pixel[0] / dimFactor));
        pixel[1] = Math.max(0, Math.round(pixel[1] / dimFactor));
        pixel[2] = Math.max(0, Math.round(pixel[2] / dimFactor));
        }
    }
    handler.savePixels(pixels, outputPath);
}
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let fila = 0; fila < catPixels.length; fila++) {
    ...
    for (let columna = 0; columna < catPixels[fila].length; columna++) {
      ...
      mergedPixel = mergedPixel.map(value => Math.min(255, Math.max(0, value)));
