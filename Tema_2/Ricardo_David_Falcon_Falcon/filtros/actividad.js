  ...
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let columna = 0; columna < pixels[fila].length; columna++) {
      const grey = Math.floor(
        (pixels[fila][columna][0] +
          pixels[fila][columna][1] +
          pixels[fila][columna][2]) /
          3
      );
      pixels[fila][columna][0] = grey;
      pixels[fila][columna][1] = grey;
      pixels[fila][columna][2] = grey;
    }
  }
...
function scaleDown() {
  ...
  for (let fila = 0; fila < pixels.length; fila += 2) {
    ...
    for (let columna = 0; columna < pixels[fila].length; columna += 2) {
      reducedRow.push(pixels[fila][columna]);
    }
    reducedImage.push(reducedRow);
  }
  pixels = reducedImage;
...
function dimBrightness(dimFactor) {
  ...
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let columna = 0; columna < pixels[fila].length; columna++) {
      pixels[fila][columna][0] = Math.floor(
        pixels[fila][columna][0] / dimFactor
      ); //  Rojo
      pixels[fila][columna][1] = Math.floor(
        pixels[fila][columna][1] / dimFactor
      ); //  Verde
      pixels[fila][columna][2] = Math.floor(
        pixels[fila][columna][2] / dimFactor
      ); //  Azul
    }
  }
...
function invertColors() {
  ...
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let columna = 0; columna < pixels[fila].length; columna++) {
      pixels[fila][columna][0] = valor - pixels[fila][columna][0];
      pixels[fila][columna][1] = valor - pixels[fila][columna][1];
      pixels[fila][columna][2] = valor - pixels[fila][columna][2];
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let fila = 0; fila < catPixels.length; fila++) {
    ...
    for (let columna = 0; columna < catPixels[fila].length; columna++) {
      let catPixel = catPixels[fila][columna];
      let dogPixel = dogPixels[fila][columna];

      let combinedPixel = [
        catPixel[0] * alphaSecond + dogPixel[0] * alphaFirst, // Rojo
        catPixel[1] * alphaSecond + dogPixel[1] * alphaFirst, // Verde
        catPixel[2] * alphaSecond + dogPixel[2] * alphaFirst, // Azul
      ];

      combinedRow.push(combinedPixel);
    }
    pixels.push(combinedRow);
  }
