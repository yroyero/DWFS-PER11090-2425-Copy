...
function greyConverter() {
    ...
    for(let i = 0; i < pixels.length; i++) {
        for(let j = 0; j < pixels[i].length; j++) {
            let pixel = pixels[i][j];
            let media = (pixel[R] + pixel[G] + pixel[B])/pixel.length;
            pixel[R] = media;
            pixel[G] = media;
            pixel[B] = media;
        }
...
function merge(alphaFirst, alphaSecond) {
    ...
    for(let i = 0; i < catPixels.length; i++) {
        ...
        for(let j = 0; j < catPixels[i].length; j++) {
            ...
            let RPixel = (catPixel[R]*alphaFirst)+(dogPixel[R]*alphaSecond);
            let GPixel = (catPixel[G]*alphaFirst)+(dogPixel[G]*alphaSecond);
            let BPixel = (catPixel[B]*alphaFirst)+(dogPixel[B]*alphaSecond);

            let pixel = [RPixel,GPixel,BPixel];
