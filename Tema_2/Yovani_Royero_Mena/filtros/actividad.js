...
function scaleDown() {
  ...
  for(let i = 0; i < pixels.length; i += 2){
    ...
    for(let j = 0; j < pixels[i].length; j += 2){
      column = [...column, pixels[i][j]]    
    }
    auxPixels = [...auxPixels, column];
  }
   pixels = [...auxPixels];
...
function merge(alphaFirst, alphaSecond) {
  ...
  for(let i = 0; i < catPixels.length; i++){
    ...
    for(let j = 0; j < catPixels[i].length; j++){
      let pixel = [];
      for(let k = 0; k < 3; k++){
        pixel[k] = catPixels[i][j][k] * alphaFirst + dogPixels[i][j][k] * alphaSecond;
      }
      column = [...column, pixel];
    }
    pixels = [...pixels, column];
  }
