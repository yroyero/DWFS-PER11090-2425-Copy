...
function redConverter() {
    ...
    for (let fila = 0; fila < pixels[fila].length; fila++) {
...
function greyConverter() {
    ...
    for (let fila = 0; fila < pixels[fila].length; fila++) {
        for (let columna = 0; columna < pixels[fila].length; columna++) {
            let media = (pixels[fila][columna][0] + pixels[fila][columna][1] + pixels[fila][columna][2]) / 3;
...
function blackAndWhiteConverter() {
    ...
    for (let fila = 0; fila < pixels.length; fila++) {
        for (let columna = 0; columna < pixels[fila].length; columna++) {
            let media = (pixels[fila][columna][0] + pixels[fila][columna][1] + pixels[fila][columna][2]) / 3;
            if (media < 128) {
                pixels[fila][columna] = [0, 0, 0]; // Negro
            } else {
                pixels[fila][columna] = [255, 255, 255]; // Blanco
            }
        }
    }
...
function scaleDown() {
    ...
    for (let fila = 0; fila < pixels.length; fila += 2) {
        ...
        for (let columna = 0; columna < pixels[fila].length; columna += 2) {
            newRow.push(pixels[fila][columna]);
        }
...
function dimBrightness(dimFactor) {
    ...
    for (let fila = 0; fila < pixels.length; fila++) {
        for (let columna = 0; columna < pixels[fila].length; columna++) {
            pixels[fila][columna][0] = Math.min(255, pixels[fila][columna][0] / dimFactor);
            pixels[fila][columna][1] = Math.min(255, pixels[fila][columna][1] / dimFactor);
            pixels[fila][columna][2] = Math.min(255, pixels[fila][columna][2] / dimFactor);
        }
    }
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let fila = 0; fila < catPixels.length; fila++) {
        ...
        for (let columna = 0; columna < catPixels[fila].length; columna++) {
            ...
            let mergedPixel = [
                Math.min(255, catPixel[0] * alphaFirst + dogPixel[0] * alphaSecond),
                Math.min(255, catPixel[1] * alphaFirst + dogPixel[1] * alphaSecond),
                Math.min(255, catPixel[2] * alphaFirst + dogPixel[2] * alphaSecond)
            ];
