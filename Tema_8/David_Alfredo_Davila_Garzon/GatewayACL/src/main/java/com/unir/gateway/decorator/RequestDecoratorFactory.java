...
/**
 * This class is responsible for creating decorators for the GatewayRequest object. It uses the ObjectMapper to convert the raw request body into a
 * GatewayRequest object. Depending on the HTTP method of the request, it creates a different decorator.
 */
@Component
...
public class RequestDecoratorFactory {
  ...
  /**
   * This method creates a decorator for the GatewayRequest object. It checks the HTTP method of the request and creates a different decorator
   * depending on the method. If the method is GET, it creates a GetRequestDecorator. If the method is POST, it creates a PostRequestDecorator. If the
   * method is neither GET nor POST, it throws an IllegalArgumentException.
   *
